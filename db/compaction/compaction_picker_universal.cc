...
Compaction* UniversalCompactionBuilder::PickCompaction() {
  ...
  for (int i = 0; i < picker_->NumberLevels() - 1; i++) {
    score_ = vstorage_->CompactionScore(i);
    start_level_ = vstorage_->CompactionScoreLevel(i);
    assert(i == 0 || score_ <= vstorage_->CompactionScore(i - 1));
    if (score_ >= 1) {
  ...
  if (start_level_inputs_.size() > 0) {
    ...
    for (FileMetaData* f : vstorage_->LevelFiles(output_level_)) {
    ...
    c = new Compaction(
      ...
      LLONG_MAX, path_id,
      GetCompressionType(ioptions_, vstorage_, mutable_cf_options_, output_level_, 1),
      GetCompressionOptions(mutable_cf_options_, vstorage_, output_level_),
      /* max_subcompactions */ 0, /* grandparents */ {}, /* is manual */ false,
      score_, false /* deletion_compaction */, compaction_reason_);
  }
